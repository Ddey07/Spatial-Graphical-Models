...
pseudo.est <- function(rho,...){
  ...
  if(is.positive.semi.definite(SIGMAhat)){
    ...
    colnames(SIGMAhat) <- c(1:(n*(1-test.prop)*p))
    rownames(SIGMAhat) <- c(1:(n*(1-test.prop)*p))
...
results <- pseudo.est(setting$rho[id],eps=10^(-3), Y=as.numeric(Y.train), mu=rep(mu.est,each=nrow(Y.train)))
...
compare.results=list(true=list(),ourmod=list(),apag=list(),apag.parsi=list())
...
dir.path <- file.path("Results", paste0("run-set_(",setting$sigma.phi[id],"_",setting$n.opt[id],")"))
dir.create(dir.path,showWarnings = FALSE)

##rho directory create
rho.ind=which(rho.seq == setting$rho[id])
# dir.create(file.path(dir.path,paste0("n_",n)),showWarnings = FALSE)
