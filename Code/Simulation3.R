...
#This is how the R file takes the argument from .sh file
#rest of your R code goes after this
...
return(list(fit=carcfit2,lik=carcfit2$logL,sigma=Rhat, time=elapsed))
} else{
  return(list(fit=NULL,lik=NULL,sigma=NULL,time=NULL))
}
}

rho.seq <- seq(0,1,len=50)

results <- pseudo.est(rho.seq[seed],eps=10^(-4))  
results$truesigma <- SIGMA
results.file <- file.path("Results", paste0("run-",seed, ".rds"))
